...
cdef class ResponseBuffer:
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef inline object _read_nullable_str_col(self, unsigned long long num_rows, char * encoding = 'utf8'):
        cdef object column = PyTuple_New(num_rows), v
        cdef unsigned long long x = 0, sz, shift
        cdef unsigned char b
        cdef char * buf
    ...
    def read_str_col(self,
                     ...
                     encoding: str = 'utf8',
                     nullable: bool = False) -> Iterable[str]:
        if nullable:
            return self._read_nullable_str_col(num_rows, encoding.encode())
