...
from clickhouse_connect.driver.compression import ZstdError, zstd_decompress, zstd_decompressor
...
def get_response_data(response: HTTPResponse) -> bytes:
    ...
    if encoding == "zstd":
        try:
            return zstd_decompress(response.data)
        except ZstdError:
            pass
    if encoding == "lz4":
