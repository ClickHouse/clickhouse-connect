...
| Upstream import                                                                    | Action  | Replacement                                                                                                                                                                                                                                                                                                                 |
|------------------------------------------------------------------------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| `from clickhouse_sqlalchemy import select as ch_select`                            | Rewrite | `from sqlalchemy import select`. Plain `select` has our chainables (`.final()`, `.sample()`, `.array_join()`, `.left_array_join()`) attached.                                                                                                                                                                               |
...
| `from clickhouse_sqlalchemy.orm.query import Query` (upstream's `ClickHouseQuery`) | Drop    | Remove `query_cls=ClickHouseQuery` from your `sessionmaker(...)` call. Upstream's subclass attaches ClickHouse chainables to `session.query(...)`; we attach the same chainables to `sqlalchemy.select`. Your own `query_cls=YourQuery` for unrelated purposes (event hooks, telemetry, custom filter helpers) still works. |
...
select(tbl).final().left_array_join(tbl.c.tags)
```

PREWHERE, LIMIT BY, and Lambda land in a later release.
...
Covers the same use case as upstream's `KeysExpressionOrColumn`. Alembic autogen renders the `Column` as its string name so generated migrations re-import cleanly.
...
Session for reads (`session.execute(select(...))`) is fine. Custom `Session` or `Query` subclasses for event hooks, telemetry, or filter helpers (not upstream's `ClickHouseQuery`, which you drop per the rewrite table) still compose: `sessionmaker(bind=engine, class_=YourSession, query_cls=YourQuery)`.

### Escaper

Upstream ships `drivers/http/escaper.py` because clickhouse-driver lacks server-side parameter binding and has to client-side-substitute values into SQL. `clickhouse-connect` has real server-side parameter binding via HTTP `{name:Type}` parameters, so `Escaper` is not needed.
...
Upstream's `make_session(engine)` wraps `sessionmaker` and wires in a custom `Query` class. We don't ship a custom `Query` class (chainables live on `Select`), so `sessionmaker(bind=engine)` covers it directly.
...
# (not upstream's ClickHouseQuery, which you drop):
...
- `Lambda(lambda x: 2*x)` AST form. The explicit `Lambda('x', column('x') * 2)` form ships in a later release. AST introspection of Python lambdas is brittle.
