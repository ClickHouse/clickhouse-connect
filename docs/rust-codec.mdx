...
The codec, its packaging, and its dependency set are experimental. Direct buffer conversion now handles non-nullable 8-64-bit integers, Float32/64, and Boolean columns. Other conversions still use PyArrow, so this change doesn't remove the PyArrow requirement for NumPy or Pandas queries.
