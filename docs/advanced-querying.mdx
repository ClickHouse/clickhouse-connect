...
For synchronous Native-format HTTP streams, context exit waits for any active read, then reads and discards the remaining response. This covers row, column, NumPy, and Pandas streams with either the Python or Rust codec. A slow query can delay cleanup. `send_receive_timeout` limits each socket read, so it does not set a deadline for context exit.

For `AsyncClient` streams that use the Rust codec, `async with` exit awaits response cleanup. If the consuming task is cancelled, cleanup finishes before cancellation is re-raised.

If the connection fails while a result is being read, a `StreamFailureError` is raised instead of silently returning a truncated result. Its message follows the client's `show_clickhouse_errors` setting.
...
| Interval*             | int                     | string            | Values are signed 64-bit counts in the interval type's unit.                                                       |
...
| Map                   | dict                    | pairs             | `dict` is the default. `pairs` returns a list of key/value tuples and preserves duplicate keys.                     |
...
### Map values with duplicate keys {#map-pairs}

ClickHouse Maps can contain duplicate keys. The default Python dictionary keeps only the last value for each key. Select the `pairs` read format to preserve every pair in the order returned by the server:
...
The format applies recursively to Maps nested inside other ClickHouse types, including Arrays, Tuples, JSON typed paths, Dynamic, Variant, and other Maps. It also works with `query_np`, `query_df`, and their streaming counterparts. To select Maps in one result column, use `column_formats={"m": {"Map": "pairs"}}`. To set it globally, use `set_read_format("Map", "pairs")`.

This is a read format. Native Map inserts still require dictionaries and don't accept pair lists. Use `query_arrow` and `insert_arrow` to round trip duplicate keys without loss. Read formats don't affect Arrow methods. As with other read-format overrides, `native_codec="rust"` falls back to Python, while `native_codec="rust_strict"` raises `NotSupportedError`.
