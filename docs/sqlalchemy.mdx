...
The server materializes the CTE only when the keyword and the `enable_materialized_cte` setting are both present, so set the setting on the statement, connection, or engine as shown in [Per-query settings](#sqlalchemy-per-query-settings). With the setting off the keyword is ignored silently: the query succeeds and returns the same rows, but the CTE is inlined again, so a forgotten setting costs performance without raising anything. Materialized CTEs require ClickHouse 26.3 or later; older servers reject the keyword as a syntax error.
