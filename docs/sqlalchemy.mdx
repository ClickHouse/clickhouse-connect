...
When SQLAlchemy inlines a bound value through `literal_binds` or `literal_execute`, ClickHouse types format it as a ClickHouse literal. This also applies through `TypeDecorator` wrappers and `with_variant()` selections. String values retain percent signs and use ClickHouse quoting even when other bound parameters remain.
