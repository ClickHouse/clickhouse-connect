...
| `show_clickhouse_errors` | bool, `"scrub"`, or None | `True` | Include server error details and symbolic exception names in client exceptions. `True` includes the request URL and server version trailer. `"scrub"` keeps the SQL error text and symbolic name but strips the host/URL and `(version ...)` trailer. `False` returns a generic message (`code` is still set). |
