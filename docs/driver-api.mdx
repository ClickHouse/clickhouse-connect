...
| `connect_timeout` | int for sync, float for async | `10` | Connection timeout in seconds. The async client accepts fractional seconds. |
...
For temporary compatibility with older host-local conversion, set `common.set_setting("naive_datetime_binding", "legacy")` before binding parameters. To preserve an instant, attach the intended `tzinfo` to the `datetime` value before passing it as a parameter. Inserts through `client.insert` interpret naive `datetime` values in the process local timezone by default. Set the global `naive_datetime_insert` setting to `"server"` to interpret them as wall time in the column timezone, or the server timezone when the column has none. See [Timezone-naive datetime objects](/integrations/language-clients/python/advanced-inserting#timezone-naive-datetime-objects).
