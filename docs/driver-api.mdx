...
| `connect_timeout` | int for sync, float for async | `10` | Timeout in seconds for establishing a new connection. Waiting for a free connection pool slot is excluded. The async client accepts fractional seconds. |
| `send_receive_timeout` | int for sync, float for async | `300` | Socket read timeout in seconds. The async client accepts fractional seconds. |
...
| `native_codec` | str or None | Global setting, `"python"` | Experimental codec for client-managed Native format traffic: `"python"`, `"rust"`, or `"rust_strict"`. The Rust values require the `clickhouse-connect-core` wheel. See [Rust codec](/integrations/language-clients/python/rust-codec). |
...
Both synchronous and asynchronous HTTP factories convert string values from DSN query parameters or `generic_args` for the following client options: `connect_timeout`, `send_receive_timeout`, `query_limit`, and `query_retries` become the numeric types shown above; `autogenerate_query_id`, `autogenerate_session_id`, and `form_encode_query_params` become booleans. Invalid string values for these options raise `ProgrammingError`.

The async factory also accepts `connector_limit=100`, `connector_limit_per_host=20`, and `keepalive_timeout=30.0` to configure its aiohttp connection pool. Pass them as direct keywords, DSN query parameters, or through `generic_args`, which is used internally for SQLAlchemy `connect_args`. String values for these connector options are converted to the documented numeric types. Invalid selected string values raise `ProgrammingError`. Explicit non-`None` keywords take precedence over `generic_args`, then the DSN. It does not accept `pool_mgr`. The synchronous chDB backend accepts `path` and `chdb_options`; see [Embedded chDB backend](#embedded-chdb-backend).
...
<Warning>
Client instances are **NOT thread-safe** when using session IDs. By default, clients have an auto-generated session ID. Concurrent queries through one client and the same session raise a `ProgrammingError` when the client detects local overlap. Named-session state and server-side session overlap checks are local to one ClickHouse server process, so a fixed `session_id` is not distributed state or a distributed mutex in ClickHouse Cloud or other load-balanced deployments.
</Warning>
...
Server-side binding works with `SELECT` queries and `INSERT ... VALUES` statements. For large plain insert batches, prefer `Client.insert`.
...
<Note>
Datetime binding treats naive values as wall time. The client formats a naive `datetime` verbatim. ClickHouse interprets it using the timezone declared in a server-side placeholder such as `{dt:DateTime('Europe/Berlin')}`, then `session_timezone` when set, then the server timezone. A timezone-aware `datetime` is converted to the timezone declared in the placeholder when present, otherwise to the server timezone reported at connection time. If the `session_timezone` setting differs from that reported server timezone, declare a timezone in the placeholder to keep the intended instant for aware values.

For temporary compatibility with older host-local conversion, set `common.set_setting("naive_datetime_binding", "legacy")` before binding parameters. To preserve an instant, attach the intended `tzinfo` to the `datetime` value before passing it as a parameter. Inserts into `DateTime` or `DateTime64` columns through `client.insert` interpret naive `datetime` values in the process local timezone by default. Set the global `naive_datetime_insert` setting to `"server"` to interpret them as wall time in the column timezone, or the server timezone when the column has none. See [Timezone-naive datetime objects](/integrations/language-clients/python/advanced-inserting#timezone-naive-datetime-objects).

Native inserts into `Date` and `Date32` columns use a Python `datetime` value's own calendar date without timezone conversion. When the same calendar date is needed for both an insert and a query parameter, pass `value.date()` explicitly. See [Date and Date32 values](/integrations/language-clients/python/advanced-inserting#date-and-date32-values).
...
A `datetime.time` or `datetime.timedelta` parameter formats as a `[-]HH:MM:SS[.ffffff]` literal for ClickHouse `Time` and `Time64` columns, in both binding styles and inside `Array` and `Tuple` values. The client adds the quotes, so don't quote the placeholder in the query. A `timedelta` may be negative and may exceed 24 hours. A pandas `Timedelta` keeps its nanoseconds and formats a nine digit fraction for `Time64(9)`. Timezone info on an aware `time` is ignored because ClickHouse `Time` has no timezone.
</Note>
...
<Note>
When the client recognizes a trailing `LIMIT 0`, it requests column metadata in JSON format. If the response contains rows, it raises `clickhouse_connect.driver.exceptions.InternalError` and does not replay the query. This can happen with `UNION`, `EXCEPT`, or `EXPLAIN` queries that end in `LIMIT 0`.

Use [`raw_query`](/integrations/language-clients/python/advanced-usage#client-rawquery-method) with an output format such as `fmt="JSON"` for these queries. It returns `bytes` for your application to decode. This behavior applies to synchronous HTTP, asynchronous HTTP, and chDB clients.
</Note>
...
The module exports the PEP 249 exception hierarchy: `Warning`, `Error`, `InterfaceError`, `DatabaseError`, `DataError`, `OperationalError`, `IntegrityError`, `InternalError`, `ProgrammingError`, and `NotSupportedError`. These are the same class objects exposed by `clickhouse_connect.driver.exceptions`, so either import path catches driver errors. The driver-specific `StreamFailureError` remains available from `clickhouse_connect.driver.exceptions` and is an `OperationalError`.
...
As a driver extension, `Connection` and `Cursor` context managers return the existing object and call `close()` on normal and exceptional exit. They don't suppress exceptions from the block.

`Cursor.execute` and `Cursor.executemany` accept additional `settings` and `query_formats` keyword arguments. `settings` passes ClickHouse settings. `query_formats` applies read formats by ClickHouse type when a statement returns rows, using the same mapping as `Client.query`. Both methods also accept the keyword-only `pyformat_encoded` argument. Its default `True` follows the DB-API `pyformat` contract. The SQLAlchemy dialect sets it to `False` when the statement compiler emitted raw percent signs, so applications normally should not set it. Parameterized `Cursor.executemany` operations execute once for each parameter set, preserving SQL binding and expression semantics. With HTTP, this sends one request per parameter set. If a later parameter set fails, earlier writes remain committed. For these `executemany` inserts, `Cursor.rowcount` is the sum of the `written_rows` values reported by ClickHouse, or `-1` when that value is unavailable. INSERT statements sent through `Cursor.execute` report `0`. The placeholder-less `INSERT INTO table (columns) VALUES` compatibility form uses Native insertion. A placeholder-less INSERT ending in `VALUES` raises `ProgrammingError` if it cannot be recognized as that compatibility form. Applications that need explicit Native bulk insertion should use `Client.insert`. `fetchone`, `fetchmany`, and `fetchall` consume the current materialized result.
...
ClickHouse does not provide traditional transactions through this HTTP interface. `Connection.commit()` and `Connection.rollback()` are no-ops, so leaving a connection context calls `Connection.close()` instead of committing or rolling back. The [session ID concurrency rules](/integrations/language-clients/python/advanced-usage#managing-clickhouse-session-ids) still apply when a connection is shared.
...
Custom exceptions, including the DB-API 2.0 exception hierarchy re-exported by `clickhouse_connect.dbapi`, are defined in `clickhouse_connect.driver.exceptions`. `DatabaseError` and `OperationalError` expose a numeric `code` attribute with the ClickHouse error code and a `name` attribute with the symbolic name such as `UNKNOWN_TABLE`, so applications can branch on `exc.code` instead of parsing the message. `code` is set even when `show_clickhouse_errors` is disabled, while `name` requires error detail (`True` or `"scrub"`). Both are `None` when unavailable, such as on transport errors. Use `show_clickhouse_errors="scrub"` when end users should see SQL errors without host or server version information. The setting also controls mid-stream `StreamFailureError` messages and generic transport messages. It governs `str(exc)` only. Transport errors are still attached as `__cause__`, and tracebacks can contain the original host, URL, or library error text.
