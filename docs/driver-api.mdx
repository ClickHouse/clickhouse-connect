...
For temporary compatibility with older host-local conversion, set `common.set_setting("naive_datetime_binding", "legacy")` before binding parameters. To preserve an instant, attach the intended `tzinfo` to the `datetime` value before passing it as a parameter. Inserts through `client.insert` still interpret naive `datetime` values in the process local timezone. See [issue #938](https://github.com/ClickHouse/clickhouse-connect/issues/938) for the plan to unify insert and parameter binding semantics.
