...
Source builds support three modes. Cython itself is always required unless the build is explicitly skipped: a missing Cython or a `cythonize()` failure fails the build in the default and required modes.
...
```bash
CLICKHOUSE_CONNECT_SKIP_CYTHON=1 pip install .   # explicit pure Python build
CLICKHOUSE_CONNECT_REQUIRE_C=1 pip install .     # extension build failure is fatal
```

A default-mode fallback produces a `py3-none-any` wheel that contains no extensions. `pip` may cache such a wheel when it was built from an index sdist and reuse it for later installs, even after the build environment is fixed. Clear it with:

```bash
pip cache remove clickhouse-connect
```

To check whether an installed copy has the extensions, import them directly:

```bash
python -c "import clickhouse_connect.driverc.buffer, clickhouse_connect.driverc.dataconv, clickhouse_connect.driverc.npconv"
```
