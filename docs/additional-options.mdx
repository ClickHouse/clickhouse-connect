...
| invalid_setting_action              | 'error' | 'drop', 'send', 'error' | Action to take when a setting known to be readonly on the server is provided (either for the client session or query). If `drop`, the setting will be ignored, if `send`, the setting will be sent to ClickHouse, if `error` a client side ProgrammingError will be raised. Settings that are not present in `system.settings` for the current user (for example custom settings declared `CHANGEABLE_IN_READONLY` on a role) are always forwarded so ClickHouse can accept or reject them. |
