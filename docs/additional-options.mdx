...
| `native_codec` | `"python"` | `"python"`, `"rust"`, `"rust_strict"` | Default codec for client-managed Native format traffic, overridable per client. The `CLICKHOUSE_CONNECT_NATIVE_CODEC` environment variable seeds this setting at import. See [Rust codec](/integrations/language-clients/python/rust-codec). |
