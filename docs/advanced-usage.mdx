...
An async client owns an aiohttp session created in one event loop. To reuse the client from another event loop, close it in the original loop, then call `await client._initialize()` in the new loop before making requests.
