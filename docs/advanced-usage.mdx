...
ClickHouse Connect supports multithreaded, multiprocess, and asyncio applications. A synchronous HTTP client can be shared across threads as long as concurrent calls do not use the same ClickHouse session. See the session discussion below. The async client runs some parsing and insert preparation in worker threads.
...
`insert_arrow` and `insert_df_arrow` run DataFrame conversion and Arrow encoding in a dedicated worker for each client, separate from streaming query readers. Each client runs one preparation operation at a time to limit temporary memory use. Close the client with `async with` or `await client.close()` when finished. PyArrow operations that hold Python's global interpreter lock can still pause other coroutines.

An async client owns an aiohttp session created in one event loop. Before moving the client to another event loop, close it in the owning loop, then call `await client._initialize()` in the new loop before making requests. If the owning loop has already closed, call `await client.close()` followed by `await client._initialize()` in the current loop. aiohttp may still report an unclosed transport when cleanup begins only after the owning loop has closed, so close before transfer when possible.
...
By default, a synchronous `Client` uses a generated session ID. `SET` statements and temporary tables persist across requests from that client only when those requests reach the same ClickHouse server process. The async factory does not generate a session ID by default. Named-session state and same-session overlap checks are process-local, and the client raises a `ProgrammingError` when it detects local overlap before sending the request. In ClickHouse Cloud or other load-balanced deployments, do not rely on a fixed `session_id` as distributed state or as a distributed mutex. Serialize before sending requests to ClickHouse if overlap matters. Use one of these patterns:
