...
This local interpretation applies to inserts only. Naive `datetime` query parameters are bound as wall time without host-local conversion. See the [Parameters argument](/integrations/language-clients/python/driver-api#parameters-argument) section. [Issue #938](https://github.com/ClickHouse/clickhouse-connect/issues/938) tracks unifying insert and parameter binding.
