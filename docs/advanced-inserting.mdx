...
The internal `DESCRIBE TABLE` uses its own server-generated query ID, while the `INSERT` keeps the `query_id` from per-insert settings or client defaults.
...
| Date                  | datetime.date or datetime.datetime | int      | Integer values are interpreted as days since 1970-01-01.                                                    |
| Date32                | datetime.date or datetime.datetime | int      | Integer values are interpreted as signed day offsets.                                                       |
...
| Map                   | dict                    |                   | The `pairs` read format doesn't change insert inputs. Use Arrow to round trip duplicate keys without loss.    |
...
#### Date and Date32 values {#date-and-date32-values}

Native inserts accept mixed Python `date` and `datetime` values in a `Date` or `Date32` column. A `datetime` contributes its own calendar date, as returned by `.date()`, without timezone conversion. This also applies inside `Nullable`, `Array`, `Tuple`, and `LowCardinality` columns.
...
This applies to Python objects, including Pandas `object` columns. Timezone-aware Pandas `datetime64` columns use the UTC calendar date. To preserve the original calendar dates, convert these values to Python `date` objects before insertion. NumPy `datetime64` values have no timezone metadata.

Datetime query parameters follow the [binding rules](/integrations/language-clients/python/driver-api#parameters-argument). An aware `datetime` is converted to the server timezone before formatting for a `Date` or `Date32` value, which can change the calendar date. Pass `value.date()` explicitly when an insert and a bound parameter must use the same calendar date.
...
Naive `datetime` query parameters use the separate `naive_datetime_binding` setting. Its default `"wall"` mode sends the wall fields without host-local conversion. See the [Parameters argument](/integrations/language-clients/python/driver-api#parameters-argument) section.
