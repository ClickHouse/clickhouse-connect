...
- DB API: an empty-result `SELECT` or `WITH` statement that begins with a SQL comment (`-- ...`, `# ...`, `#! ...`, or `/* ... */`) now populates `cursor.description`. The metadata re-query that recovers column names for an empty result was gated on the statement starting with `SELECT` or `WITH`, but a leading comment hid that keyword and silently skipped the re-query, leaving `cursor.description` empty. The classification now looks past any leading comment, while the re-query still wraps the original statement unchanged. Closes [#909](https://github.com/ClickHouse/clickhouse-connect/issues/909).
