...
- A query whose final `;` is followed by whitespace or an SQL comment is no longer rejected with `Code: 62 ... Multi-statements are not allowed`. The client strips a trailing `;` before appending its `FORMAT` clause, but the previous `rstrip(";")` only removed a bare `;`, so anything after it (a newline as in `SELECT 1;\n`, a trailing space, or a `-- ...` or `/* ... */` comment) left the `;` in place and turned the appended `FORMAT` into a second statement. Text before the terminator, including comments, is preserved, and a `;` inside a string literal or a trailing comment is not treated as a terminator. Applies to both sync and async clients. Closes [#903](https://github.com/ClickHouse/clickhouse-connect/issues/903).
