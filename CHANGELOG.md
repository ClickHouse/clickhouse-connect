...
- Naive (timezone-unaware) `datetime` values bound as query parameters are no longer shifted by the client host's local timezone. A naive value previously went through `astimezone(server_tz)`, which treats it as host-local time, so on any non-UTC host the parameter was silently offset and matched the wrong rows. Naive datetimes are now formatted verbatim; timezone-aware values are still converted to the server timezone.
