...
- Arrow query methods now detect a server exception raised part way through a streamed response. When a query fails after the HTTP interface has already committed a `200 OK` and started streaming rows, ClickHouse appends an in-band exception to the body. The native read path already surfaced this, but the Arrow paths handed the raw bytes to PyArrow and never consulted it, so a mid-stream failure surfaced as a truncated or garbled Arrow stream or a raw transport error (`urllib3.ProtocolError` / `aiohttp.ClientPayloadError`) instead of the actual server error. `query_arrow`, `query_arrow_stream`, and `query_df_arrow_stream` on both the sync and async clients now raise the clean ClickHouse error, matching the native streaming path. Closes [#913](https://github.com/ClickHouse/clickhouse-connect/issues/913).
