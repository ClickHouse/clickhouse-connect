...
- The DB-API `Cursor` metadata fallback no longer fails with `DatabaseError` code 62 when a `SELECT` or `WITH` operation ends with a SQL comment. When a result carries no column names, the cursor introspects `cursor.description` by wrapping the operation in `SELECT * FROM (...) LIMIT 0`. A trailing single line comment (`--` or `#`) previously commented out the closing parenthesis, and a trailing `;` left a statement terminator inside the subquery, both of which the server rejected. Trailing whitespace, semicolons, and comments are now removed before wrapping, while comment markers and semicolons inside string literals or quoted identifiers are preserved. Closes [#907](https://github.com/ClickHouse/clickhouse-connect/issues/907).
