...
- `LowCardinality` inserts of `Float32` and `Float64` columns now preserve negative zero. The block dictionary deduplicated rows with a plain Python `dict`, and because `0.0 == -0.0` and the two hash alike, a `-0.0` row reused the `+0.0` dictionary entry and was sent as `+0.0`. Negative zero now takes its own dictionary entry, while repeated negative zeros still deduplicate to a single entry. Closes [#1029](https://github.com/ClickHouse/clickhouse-connect/issues/1029).
