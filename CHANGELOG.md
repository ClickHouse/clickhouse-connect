...
- DB API `Cursor.description` now reports each column's real `null_ok` flag (the 7th tuple element) instead of a hardcoded `True`. The cursor kept only the type name string from the query layer and discarded the `ClickHouseType`, so tooling that reads `description[i][6]` for schema inference saw every column as nullable, including `UInt32`, `String`, and primary-key columns. `Cursor.execute` and `Cursor.executemany` now retain the `ClickHouseType` objects and `description` reads `type.nullable`. The `type_code` field stays the type name string and is now consistent between `execute` and `executemany`. Closes [#902](https://github.com/ClickHouse/clickhouse-connect/issues/902).
