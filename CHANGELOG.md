...
### Bug Fixes
- Fix `ValueError: data type not inexact` raised when constructing `DateTime` or `DateTime64` columns with ClickHouse `Fixed/UTC±HH:MM:SS` timezone strings (e.g. `DateTime('Fixed/UTC+05:30:00')`). These fixed-offset timezone strings are not recognised by `pytz`, causing an `UnknownTimeZoneError` that was previously unhandled. A new `parse_timezone()` helper in `driver/tzutil.py` detects this format and builds a stdlib `datetime.timezone` from the parsed offset, falling back to `pytz` for all other strings. Closes [#702](https://github.com/ClickHouse/clickhouse-connect/issues/702)
