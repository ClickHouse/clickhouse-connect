...
- A `decimal.Decimal` passed through client-side parameter substitution (`%s` or `%(name)s`) is now inlined as a quoted string literal instead of a bare numeric literal. Previously it had no branch in `format_query_value`, so the value was substituted unquoted and ClickHouse parsed it as `Float64`. For high-precision values such as `Decimal(38, 10)` this silently lost precision, so a comparison like `WHERE col < %s` was evaluated in `Float64` and could drop rows that should match. The quoted literal is coerced by the server to the target `Decimal` type without going through `Float64`, and `Decimal` values nested in `Array`, `Tuple`, and `Map` parameters are quoted the same way. The server-side bind parameter path (`{name:Type}`) is unchanged, as it already passes the value unquoted to be parsed against the declared type. Reported upstream as [ClickHouse/clickhouse-java#2762](https://github.com/ClickHouse/clickhouse-java/issues/2762).
