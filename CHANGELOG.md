...
### Improvements
- Added a small pluggable execution-backend mechanism. `get_client(backend="...")` resolves an out-of-tree backend registered through the `clickhouse_connect.backends` entry-point group, so an alternative engine (for example chDB's in-process embedded ClickHouse) can sit behind the existing `Client` without any backend-specific code, tests, or CI dependency living in this repository. The default behavior is unchanged: with no `backend` argument (or `backend="http"`) the built-in HTTP transport is used and is byte-identical to before. A missing backend raises `BackendNotInstalledError` with an install hint; clickhouse-connect never installs a backend itself. New modules: `driver/backend.py` (the `Backend` protocol and capability flags) and `driver/registry.py` (entry-point discovery).
