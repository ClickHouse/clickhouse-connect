...
- Added an experimental native_codec client option that selects the codec for client-managed FORMAT Native query decode and insert encode. python is the default and uses the existing Python and Cython codec. rust prefers the compiled _ch_core codec and routes unsupported query options or insert types to the Python codec. rust_strict raises instead of routing. A matching common setting and the CLICKHOUSE_CONNECT_NATIVE_CODEC environment variable seed the default, with precedence kwarg over common setting over environment variable. The codec covers query, query_np, query_df, their block and row stream variants, and inserts including insert_df, with dtypes matching the Python codec. The Arrow methods stay on FORMAT Arrow and are unaffected. Malformed Native payloads detected by the Rust codec raise DataError. Requires a build that includes the _ch_core extension. This is early access for benchmarking and is not yet a supported path.

### Bug Fixes
- Correct QBit Native wire byte ordering for dimensions greater than 8. Earlier releases wrote each eight-element byte group in forward order instead of the reverse order required by ClickHouse. Those rows were self-consistent when read by the same client, but the server interpreted their element positions incorrectly, including in `L2DistanceTransposed`. QBit rows with dimensions greater than 8 that were inserted by an affected client release should be re-inserted after upgrading. Closes [#866](https://github.com/ClickHouse/clickhouse-connect/issues/866).
