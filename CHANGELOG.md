...
### Improvements
- Added a small pluggable execution-backend mechanism. A non-HTTP DSN scheme passed to `get_client` selects an out-of-tree backend registered through the `clickhouse_connect.backends` entry-point group, so an alternative engine (for example chDB's in-process embedded ClickHouse) can sit behind the existing `Client` without any backend-specific code, tests, or CI dependency living in this repository: `get_client("chdb://memory")` and `get_client("chdb:///path/to/db")` route to the chDB backend. The default behavior is unchanged: an HTTP / HTTPS DSN (or no DSN at all) uses the built-in HTTP transport, byte-identical to before. A missing backend raises `BackendNotInstalledError` with an install hint; clickhouse-connect never installs a backend itself. New modules: `driver/backend.py` (the `Backend` protocol and capability flags) and `driver/registry.py` (entry-point discovery).
