...
### Breaking Changes
- Dropped the `pytz` dependency in favor of the standard library `zoneinfo` module. The minimum supported Python version (3.10) already ships `zoneinfo`, so no compatibility shim is needed. On Windows, `tzdata` is now pulled in automatically to provide the IANA timezone database that pytz previously bundled. On Linux and macOS, the system zoneinfo database is used; on slim container images that ship without it, install the new optional extra via `pip install clickhouse-connect[tzdata]`. Error behavior changes: when an unknown timezone string is supplied to `query_tz`, `column_tzs`, or is reported by the server, the raised/caught exception is now `zoneinfo.ZoneInfoNotFoundError` instead of `pytz.exceptions.UnknownTimeZoneError`, and the user-visible `ProgrammingError` and log messages now suggest installing the `tzdata` extra when resolution fails. Closes [#714](https://github.com/ClickHouse/clickhouse-connect/issues/714).
