...
- DB-API `Cursor.executemany` now recognizes the INSERT forms the server accepts before sending the rows as a bulk insert. The optional `TABLE` keyword in `INSERT INTO TABLE tbl VALUES ...` was parsed as the table name, which dropped the column list as well, and an `INSERT INTO [TABLE] FUNCTION ...` table function target was parsed as a table named `FUNCTION` or `TABLE` instead of running row by row. An INSERT formatted over several lines silently lost the bulk insert fast path and ran one query per row, because the statement pattern only captured the first line. Double-quoted column names such as `INSERT INTO tbl ("id", "name") VALUES ...` kept their quotes, which raised `ProgrammingError: Unrecognized column` with sequence rows and lost the fast path with dict rows, so `unescape_identifier` now removes double-quote as well as backtick quoting, matching what `quote_identifier` already accepts. A statement whose target is not followed directly by `VALUES`, such as an `INSERT SELECT` or an INSERT with a `SETTINGS` or `FORMAT` clause, now stays on the row-by-row path so the rest of the statement is not dropped, which also means those statements report the `rowcount` of that path instead of the written rows of a bulk insert. Closes [#932](https://github.com/ClickHouse/clickhouse-connect/issues/932).
