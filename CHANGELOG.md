...
- Naive `datetime` query parameters now bind as wall time by default. Client-side `%` parameters, server-side `{name:DateTime}` parameters, and `DT64Param` values send naive datetimes verbatim. UTC hosts with UTC targets are unchanged. On non-UTC hosts with UTC targets, server-side parameters and `DT64Param` values no longer shift while plain client-side `%` parameters were already verbatim because of the old UTC guard. For non-UTC targets, naive values now keep their wall fields instead of preserving the instant implied by the client local timezone. Timezone-aware datetimes still preserve instants by converting to the target bind timezone. Set `common.set_setting("naive_datetime_binding", "legacy")` to restore the previous host-local conversion behavior. Code that meant a naive value to represent a specific instant should attach the intended `tzinfo` before binding.
