...
- Reading a `JSON` column that declares one path as both a value and the parent of a deeper path, such as `JSON(a Int64, a.b Nullable(Int64))`, no longer aborts the query with `TypeError: 'int' object does not support item assignment`. Typed paths are read in lexicographic order, so the scalar was always written before the deeper path walked through it. A deeper path holding nothing now gives way and the scalar survives; a deeper path carrying a value replaces the scalar, matching what an overlapping dynamic path already did. Closes [#997](https://github.com/ClickHouse/clickhouse-connect/issues/997).
