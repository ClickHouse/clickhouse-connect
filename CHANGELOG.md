...
- SQLAlchemy literals of a ClickHouse type now double a literal `%` for the DB-API `pyformat` paramstyle, matching what SQLAlchemy's own literal processors do. A rendered value containing `%`, for example `literal("100%", type_=String(), literal_execute=True)`, previously reached the driver as a single `%`, so a statement that still carried a bound parameter failed with `ValueError: unsupported format character` during parameter interpolation. The `%` is unescaped again when the query is finalized, so the SQL sent to the server is unchanged. Literals are left with a single `%` under `server_side_params`, which turns percent doubling off. Closes [#966](https://github.com/ClickHouse/clickhouse-connect/issues/966).
