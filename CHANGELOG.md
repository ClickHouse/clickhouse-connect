...
- SQLAlchemy `Nullable()` and `LowCardinality()` are now annotated as returning an instance of the wrapped type instead of the `ChSqlaType` mixin, matching their runtime behavior. `Column("hostname", LowCardinality(String))` no longer fails `mypy --strict` with an `arg-type` error. Closes [#1033](https://github.com/ClickHouse/clickhouse-connect/issues/1033).
