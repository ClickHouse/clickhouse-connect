...
- The Rust codec now converts non-nullable 8-64-bit integers, Float32/64, Boolean, BFloat16, Interval, Date/Date32, DateTime/DateTime64, and Time/Time64 columns to NumPy/Pandas directly from its decoded buffers. Nullable BFloat16, Time/Time64, and extended Pandas nullable integer, float, and Interval output also use direct buffers. Output dtypes are unchanged. Other column converters still require PyArrow, so the existing PyArrow requirement for Rust NumPy/Pandas queries remains.
