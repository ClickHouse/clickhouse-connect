...
- The DB-API `Connection` and `Cursor` now implement the context manager protocol, so `with connect(...) as connection:` and `with connection.cursor() as cursor:` work and close the connection or cursor on exit. ClickHouse has no client-side transactions (the DB-API `commit` and `rollback` are no-ops), so exiting the connection context closes the connection rather than ending a transaction.
