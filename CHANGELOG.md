...
- The Rust codec now converts non-nullable 8-64-bit integers, Float32/64, Boolean, BFloat16, and Interval columns to NumPy/Pandas directly from its decoded buffers. Nullable BFloat16 and extended Pandas nullable integer, float, and Interval output also use direct buffers. Output values, dtypes, and null handling are unchanged. Other column converters still require PyArrow, so the existing PyArrow requirement for Rust NumPy/Pandas queries remains.
