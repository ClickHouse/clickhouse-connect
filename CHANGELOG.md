...
- SQLAlchemy DDL now renders table, dictionary, and column comments, and string column defaults, with ClickHouse string escaping. These four literals were rendered with SQLAlchemy's generic string type, which only doubles single quotes and leaves backslashes untouched. ClickHouse treats a backslash as an escape character, so a comment such as `path C:\\temp` was silently stored as `path C:<tab>emp`, and a comment ending in a backslash made the server reject the statement with `Single quoted string is not closed`.
