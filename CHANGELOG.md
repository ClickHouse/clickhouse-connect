...
- DB-API `Cursor.executemany` now parses the target table of an INSERT as an identifier chain instead of splitting on the first space or open paren. ClickHouse accepts whitespace around the dot of a database qualified name, so `INSERT INTO db . tbl VALUES (%s)` used to parse the table as just `db` and send the rows to the database name, and a quoted name containing a space was truncated the same way. A statement whose table name cannot be parsed now falls back to row-by-row execution instead of raising `IndexError`. Closes [#930](https://github.com/ClickHouse/clickhouse-connect/issues/930).
