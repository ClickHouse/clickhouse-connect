...
- A query whose final `;` is followed by whitespace or a comment is no longer rejected with `Code: 62 ... Multi-statements are not allowed`. The client strips a trailing `;` before appending its `FORMAT` or `LIMIT` clause, but the previous `rstrip(";")` only removed a bare `;`, so anything after it (a newline as in `SELECT 1;\n`, a trailing space, or a comment) left the `;` in place and turned the appended clause into a second statement. The terminator is now found with a single linear scan that follows the server lexer: `--`, `//`, `# ` and `#!` line comments, nested `/* */` block comments, `''`, `""` and ` `` ` quoting, and `$tag$` heredocs, so a `;` inside a comment, a string literal, a quoted identifier or a heredoc is not treated as a terminator. Text before the terminator, including comments, is preserved, and a query with an unterminated comment or quote is passed through unchanged so the server reports the syntax error. Applies to `query`, `command`, and `raw_query` on both the sync and async clients. Closes [#903](https://github.com/ClickHouse/clickhouse-connect/issues/903).
