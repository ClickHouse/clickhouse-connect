...
@pytest.mark.asyncio
async def test_close_rejects_queued_preparation_without_waiting(arrow_client, mocker):
    loop = asyncio.get_running_loop()
    started = asyncio.Event()
    finished = asyncio.Event()
    release = threading.Event()
...
async def test_close_during_conversion_rejects_stale_insert(arrow_client, polars_frame, mocker, reinitialize):
    loop = asyncio.get_running_loop()
    started = asyncio.Event()
    finished = asyncio.Event()
    release = threading.Event()
