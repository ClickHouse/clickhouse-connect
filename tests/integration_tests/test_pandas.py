...
from clickhouse_connect.driver.options import arrow, np, pd
...
def test_pandas_basic(param_client: Client, call, test_table_engine: str):
    ...
    call(param_client.command, f"CREATE TABLE test_system_insert_pd as system.tables Engine {test_table_engine} ORDER BY (database, name)")
...
def test_pandas_nulls(param_client: Client, call, table_context: Callable):
    ...
    with table_context("test_pandas_nulls_bad", ["key String", "num Int32", "flt Float32", "str String", "dt DateTime", "day_col Date"]):
        with pytest.raises(DataError):
            call(param_client.insert_df, "test_pandas_nulls_bad", df, column_names=insert_columns)

    with table_context(
        "test_pandas_nulls_good",
        [
            "key String",
            "num Nullable(Int32)",
            "flt Nullable(Float32)",
            "str Nullable(String)",
            "dt Nullable(DateTime('America/Denver'))",
...
def test_pandas_context_inserts(param_client: Client, call, table_context: Callable):
    with table_context("test_pandas_multiple", null_ds_columns, null_ds_types):
        ...
        next_df = pd.DataFrame(
            [["key4", -415, None, "value4", datetime(2022, 7, 4, 15, 33, 4, 5233), date(1999, 12, 31)]], columns=null_ds_columns
        )
...
def test_pandas_enums(param_client: Client, call, table_context: Callable):
    columns = [
        "key String",
        "value Enum8('Moscow' = 0, 'Rostov' = 1, 'Kiev' = 2)",
...
def test_pandas_datetime64(param_client: Client, call, table_context: Callable):
    ...
    with table_context(
        ...
        now = datetime.now()
        df = pd.DataFrame(
            [["key1", now, now, now], ["key2", nano_timestamp, milli_timestamp, chicago_timestamp]],
...
def test_pandas_row_df(param_client: Client, call, table_context: Callable):
    with table_context("test_pandas_row_df", ["key UInt64", "dt DateTime64(6)", "fs FixedString(5)"]):
        df = pd.DataFrame(
            {"key": [1, 2], "dt": [pd.Timestamp(2023, 5, 4, 10, 20), pd.Timestamp(2023, 10, 15, 14, 50, 2, 4038)], "fs": ["seven", "bit"]}
        )
        df = df.iloc[1:]
        source_df = df.copy()
...
def test_pandas_small_blocks(test_config: TestConfig, param_client: Client, call):
    ...
    res = call(param_client.query_df, "SELECT number, randomString(512) FROM numbers(1000000)", settings={"max_block_size": 250})
...
def test_pandas_string_to_df_insert(param_client: Client, call, table_context: Callable):
    ...
    with table_context(
        ...
        result_df = call(param_client.query_df, "SELECT * FROM test_pandas_string_to_df_insert ORDER BY id")
