...
def test_arrow(param_client: Client, call, table_context: Callable):
    ...
    arrow_table = call(param_client.query_arrow, "SELECT number from system.numbers LIMIT 500", settings={"max_block_size": 50})
...
def test_arrow_map(param_client: Client, call, table_context: Callable):
    ...
    with table_context(
        ...
        data = [[date(2023, 10, 15), "C1", {"k": 2.5, "d": 0, "j": 0}], [date(2023, 10, 16), "C2", {"k": 3.5, "d": 0, "j": -0.372}]]
        call(
            param_client.insert,
            "test_arrow_map",
            data,
            column_names=("trade_date", "code", "kdj"),
            settings={"insert_deduplication_token": "10381"},
        )
        arrow_table = call(param_client.query_arrow, "SELECT * FROM test_arrow_map ORDER BY trade_date", use_strings=True)
