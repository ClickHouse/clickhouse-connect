...
ext_settings = {"input_format_allow_errors_num": 10, "input_format_allow_errors_ratio": 0.2}
...
def test_external_simple(param_client: Client, call):
    ...
    result = call(param_client.query, "SELECT * FROM movies ORDER BY movie", external_data=data, settings=ext_settings).result_rows
...
def test_external_multiple(param_client: Client, call):
    ...
    data = ExternalData(movies_file, fmt="CSVWithNames", structure=["movie String", "year UInt16", "rating Decimal32(3)"])
    ...
    result = call(
        ...
        "SELECT * FROM actors;",
        external_data=data,
        settings={"input_format_allow_errors_num": 10, "input_format_allow_errors_ratio": 0.2},
    ).result_rows
...
def test_external_binary(param_client: Client, call):
    ...
    data = ExternalData(file_name="actors.csv", data=actors, structure="name String, birth_year UInt16, movie String")
