...
def test_query(test_driver: BaseClient):
    result = test_driver.query('SELECT * FROM system.tables')
    assert len(result.result_set) > 0


def test_command(test_driver: BaseClient):
    version = test_driver.command('SELECT version()')
    assert version.startswith('2')


def test_insert(test_driver: BaseClient):
