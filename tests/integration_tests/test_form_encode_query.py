...
def test_form_encode_query_basic(client_factory, call, table_context: Callable):
    ...
    with table_context("test_form_encode", ["id UInt32", "name String", "value Float64"]):
        ...
        result = call(form_client.query, "SELECT * FROM test_form_encode WHERE id = {id:UInt32}", parameters={"id": 2})
...
def test_form_encode_with_arrays(client_factory, call, table_context: Callable):
    ...
    with table_context("test_form_arrays", ["id UInt32", "tags Array(String)"]):
        ...
        result = call(form_client.query, "SELECT * FROM test_form_arrays WHERE has(tags, {tag:String})", parameters={"tag": "tag3"})
        assert result.row_count == 2

        ids = [1, 3]
        result = call(form_client.query, "SELECT * FROM test_form_arrays WHERE id IN {ids:Array(UInt32)}", parameters={"ids": ids})
        assert result.row_count == 2
...
def test_form_encode_raw_query(client_factory, call):
    ...
    result = call(form_client.raw_query, "SELECT {a:Int32} + {b:Int32} as sum", parameters={"a": 10, "b": 20})
...
def test_form_encode_nullable_params(client_factory, call):
    ...
    result = call(form_client.query, "SELECT {val:Nullable(String)} IS NULL as is_null", parameters={"val": None})
    assert result.first_row[0] == 1

    result = call(form_client.query, "SELECT {val:Nullable(String)} as value", parameters={"val": "test_value"})
...
def test_form_encode_schema_probe_query(client_factory, call, table_context: Callable):
    ...
    with table_context("test_schema_probe", ["id UInt32", "name String", "value Float64"]):
        result = call(form_client.query, "SELECT * FROM test_schema_probe WHERE id = {id:UInt32} LIMIT 0", parameters={"id": 1})
