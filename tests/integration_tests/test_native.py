...
def test_bare_datetime64(param_client: Client, call, table_context: Callable):
    with table_context('bare_datetime64_test', ['key UInt32', 'dt64 DateTime64']):
        call(param_client.insert, 'bare_datetime64_test',
...
def test_nulls(param_client: Client, call, table_context: Callable):
    with table_context('nullable_test', ['key UInt32', 'null_str Nullable(String)', 'null_int Nullable(Int64)']):
        call(param_client.insert, 'nullable_test', [[1, None, None],
...
def test_old_json(param_client: Client, call, table_context: Callable):
    ...
    with table_context('old_json_test', [
        ...
        call(param_client.insert, 'old_json_test', [
            [5, jv1, -44, None],
            [20, None, 5200, njv2],
            [25, jv3, 7302, njv3]])

        result = call(param_client.query, 'SELECT * FROM old_json_test ORDER BY key')
        ...
        call(param_client.insert, 'native_json_test', [[999, '{"key4": 283, "value.2": "str_value"}', 77, '{"nk1":53}']])
        result = call(param_client.query, 'SELECT value.key4, null_value.nk1 FROM native_json_test ORDER BY key')
...
def test_read_formats(param_client: Client, call, test_table_engine: str):
    ...
    call(param_client.command, 'CREATE TABLE read_format_test (key Int32, uuid UUID, fs FixedString(10), ipv4 IPv4,' +
    ...
    # Test query formats
    result = call(param_client.query, 'SELECT * FROM read_format_test', query_formats={'IP*': 'string',
                                                                                'tup': 'json'}).result_set
    assert result[1][3] == '10.44.75.20'
    ...
    # Test column formats
    result = call(param_client.query, 'SELECT * FROM read_format_test', column_formats={'ipv4': 'string',
