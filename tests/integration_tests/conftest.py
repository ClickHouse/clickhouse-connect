...
def test_config_fixture() -> Iterator[TestConfig]:
    ...
    test_database = os.environ.get(
        "CLICKHOUSE_CONNECT_TEST_DATABASE", f"ch_connect__{random.randint(100000, 999999)}__{int(time.time() * 1000)}"
    )
...
async def test_native_async_client_fixture(test_config: TestConfig) -> AbstractAsyncContextManager:
