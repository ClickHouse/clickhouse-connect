...
def test_params(param_client: Client, call, table_context: Callable):
    result = call(param_client.query, 'SELECT name, database FROM system.tables WHERE database = {db:String}',
                               parameters={'db': 'system'})
    assert result.first_item['database'] == 'system'
    if param_client.min_version('21'):
        result = call(param_client.query, 'SELECT name, {col:String} FROM system.tables WHERE table ILIKE {t:String}',
    ...
    with table_context('test_bind_params', ['key UInt64', 'dt DateTime', 'value String', 't Tuple(String, String)']):
        call(param_client.insert, 'test_bind_params',
                           [[1, first_date, 'v11', ('one', 'two')],
                            [2, second_date, 'v21', ('t1', 't2')],
                            [3, datetime.now(), 'v31', ('str1', 'str2')]])
        result = call(param_client.query, 'SELECT * FROM test_bind_params WHERE dt = {dt:DateTime}',
                                   parameters={'dt': second_date})
        assert result.first_item['key'] == 2
        result = call(param_client.query, 'SELECT * FROM test_bind_params WHERE dt = %(dt)s',
                                   parameters={'dt': first_date})
        assert result.first_item['key'] == 1
        result = call(param_client.query, "SELECT * FROM test_bind_params WHERE value != %(v)s AND value like '%%1'",
                                   parameters={'v': 'v11'})
        assert result.row_count == 2
        result = call(param_client.query, 'SELECT * FROM test_bind_params WHERE value IN %(tp)s',
                                   parameters={'tp': ('v18', 'v31')})
        assert result.first_item['key'] == 3

    result = call(param_client.query, 'SELECT number FROM numbers(10) WHERE {n:Nullable(String)} IS NULL',
...
def test_datetime_64_params(param_client: Client, call):
    ...
    result = call(param_client.query, 'SELECT {a1:Array(DateTime64(6))}',
