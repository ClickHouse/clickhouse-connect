...
from clickhouse_connect.tools.datagen import RandomValueDef, random_ascii_str, random_col_data
...
def random_type(depth: int = 0, low_card_perc: float = LOW_CARD_PERC, nullable_perc: float = NULLABLE_PERC, parent_type: str = None):
    ...
    low_card_ok = True
    while (
        base_type in unsupported_types
        or (depth >= NESTED_DEPTH and base_type in nested_types)
        or parent_type == "Nested"
        and base_type in ("Int128", "Int256", "UInt256", "UInt126")
    ):
        base_type = random.choices(all_types, all_weights)[0]
...
def build_nested_type(base_type: str, depth: int):
    ...
    if base_type == "Map":
        ...
        while key.python_type not in (str, int) or (key.python_type is int and key.low_card):
            key = random_type(1000, nullable_perc=0)
...
def random_data(col_types: Sequence[ClickHouseType], num_rows: int = 1, server_tz: tzinfo = timezone.utc):
...
def bytes_source(data: str | bytes, chunk_size: int = 256, cls: type = ResponseBuffer):
